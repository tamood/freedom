...
class FPGAChip(implicit val p: Parameters) extends NexysA7Shell {
  ...
  ddrpads <> coreplex.ddrmigio
    ...
    val iof_20 = coreplex.iof(2).get.iof_0
    ...
    (0 to 3) foreach {i => 
		IOBUF(QSPI_DQ(i), spif_pins(0).dq(i))
		}
    ...
    coreplex.debug.foreach { debugio => 
		...
		debugio.systemjtag.foreach { sj =>
			...
			val btunnel_jtag = IOBUF(JB(1))
			PULLUP(JB(1))

			sj.jtag.TCK := Mux(btunnel_jtag, btunnel.jtag_tck.asBool, IOBUF(JB(10))).asClock
			sj.jtag.TMS := Mux(btunnel_jtag, btunnel.jtag_tms, IOBUF(JB(7)))
			sj.jtag.TDI := Mux(btunnel_jtag, btunnel.jtag_tdi, IOBUF(JB(8)))
			btunnel.jtag_tdo := sj.jtag.TDO.data
			btunnel.jtag_tdo_en := sj.jtag.TDO.driven
